...
export const BulkAssetRowSchema = z.object({
  ...
  registrationDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be YYYY-MM-DD').refine(s => !isNaN(Date.parse(s)), { message: 'Invalid calendar date' }),
  salvageValue: z.number().min(0).default(0),
  life: z.number().int().min(1).max(100),
  depreciationMethod: z.enum(['Straight Line', 'Reducing Balance', 'Sum of Years Digits']).default('Straight Line'),
  location: z.string().min(1).max(200).trim(),
  subLocation: z.string().max(200).trim().optional(),
...
export const SyncDepartmentsSchema = z.object({
  locationBranches: z.record(z.string().max(100), z.array(z.string().max(200)).max(200)),
  codes: z.record(z.string().max(20)),
});
