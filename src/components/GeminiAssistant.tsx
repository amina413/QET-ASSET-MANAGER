...
const GeminiAssistant: React.FC<GeminiAssistantProps> = ({ assets = [], isOpen, onClose }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: genMsgId(), role: 'model', text: 'Hello! I am your QET Asset Assistant. Ask me about asset values, disposal policies, or drafting reports. You can also upload images, documents, or use voice input.' }
  ]);
  ...
  // SpeechRecognition types not in standard TS lib — use a minimal structural interface
  ...
  const appendMessage = (msg: Omit<Message, 'id'>) => {
    setMessages(prev => {
      const updated = [...prev, { ...msg, id: genMsgId() }];
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-96 h-[500px] bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 flex flex-col animate-slideIn overflow-hidden font-sans">
          ...
          <div className="bg-gradient-to-r from-qet-800 to-accent-500 p-4 text-white flex justify-between items-center">
            ...
            <button onClick={onClose} className="hover:bg-white/20 p-1 rounded" aria-label="Close AI assistant">
          ...
          <div className="p-3 bg-white border-t border-slate-100">
            <div className="flex gap-2 mb-2">
              ...
              >
                {isRecording ? <MicOff size={18} aria-hidden /> : <Mic size={18} aria-hidden />}
              </button>
              <button
                onClick={toggleVoiceOutput}
                aria-label={isSpeaking ? 'Stop voice output' : 'Enable voice output'}
