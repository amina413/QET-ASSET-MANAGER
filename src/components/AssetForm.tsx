...
import { downloadCsv, parseCsv, rowsToCsv } from '@/utils/csv';
...
const BARCODE_PREVIEW_BARS = Array.from({ length: 40 }, () => ({
...
const AssetForm: React.FC<AssetFormProps> = ({ onBack, currentUser, users = [], onSuccess }) => {
  ...
  const resolveCustodianId = (custodian: string, assignedUser?: string): string | null => {
    const target = (custodian === 'Individual' ? assignedUser : custodian)?.trim().toLowerCase();
    if (!target) return null;

    const matchedUser = users.find(user =>
      user.id === target ||
      user.name.toLowerCase() === target ||
      user.email?.toLowerCase() === target ||
      user.department.toLowerCase() === target
    );

    return matchedUser?.id ?? null;
  };

  const uploadAssetImage = async (assetId: string, file: File): Promise<string | null> => {
    const upload = await assetService.createImageUploadUrl(assetId, file);
    ...
    if (!uploadRes.ok) return 'Failed to upload asset image.';

    const update = await assetService.updateImage(assetId, upload.data.publicUrl);
    return update.success ? null : update.error;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const custodianId = resolveCustodianId(formData.custodian, formData.assignedUser);
      if (!custodianId) {
        setErrors(prev => ({
          ...prev,
          custodian: 'Select a custodian that maps to an active user record.',
        }));
        return;
      }
      ...
      if (result.success) {
        if (imageFile) {
          const imageError = await uploadAssetImage(result.data.assetId, imageFile);
          if (imageError) {
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        toast("Error parsing file. Please ensure it is a valid CSV file.", 'error');
      } finally {
  ...
  const revalidateRows = (rows: BulkAssetRow[]): BulkAssetRow[] => {
    ...
    return rows.map(row => {
      ...
      if (row.custodian && !resolveCustodianId(row.custodian, row.assignedUser)) {
        errors.push('Custodian does not match an active user, email, or department');
      }
  ...
  const handleBulkImport = async () => {
    ...
    const rows = validRows.map(row => ({
      ...
      condition: row.condition || 'New',
      custodianId: resolveCustodianId(row.custodian, row.assignedUser) ?? undefined,
    }));

    let result: Awaited<ReturnType<typeof assetService.bulkCreate>>;
    try {
      result = await assetService.bulkCreate(rows);
    } finally {
      setIsSubmitting(false);
    }

    if (!result.success) {
      toast(result.error || "Failed to save imported assets. Please try again.", 'error');
      return;
    }
              ...
              <div className="flex flex-col items-center justify-center py-6 animate-fadeIn">
                ...
                <div id="asset-tag-card" className="bg-slate-50 border-2 border-slate-800 rounded-xl p-6 w-full max-w-sm mb-8 relative">
                  ...
                  <div className="text-center">
                    ...
                    <div className="h-48 bg-white border border-slate-300 my-4 flex items-center justify-center overflow-hidden px-4 rounded-sm">
                      {showQrCode ? (
                        ...
                        <div className="flex items-end h-16 space-x-[3px] w-full justify-center opacity-90">
                          {BARCODE_PREVIEW_BARS.map((bar, i) => <div key={i} className="bg-black" style={{ width: bar.width, height: bar.height }}></div>)}
                        </div>
                      )}
                    </div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-2 uppercase">{LABEL_OWNER}</p>
                  </div>
                </div>
