...
export const calculateDepreciationSchedule = (input: DepreciationInput): DepreciationYear[] => {
    ...
    if (isNaN(regDate.getTime()) || regDate.getFullYear() < 1900 || regDate.getFullYear() > 2100) {
        throw new Error(`Invalid registration date: "${registration_date}"`);
    }
    ...
    let remainingUsefulLifeDays = useful_life * 365; // NOTE: Uses 365-day years; for production-grade accuracy compute exact dates instead of fixed-day multiplication
...
export const calculateMonthlyDepreciationSchedule = (input: DepreciationInput): DepreciationMonth[] => {
    ...
    for (let i = 0; i <= totalMonths; i++) {
        ...
        if (i === 0) {
            ...
            monthlyExpense = (baseAnnual / 12) * (daysActive / daysInMonth); // Pro-rata based on actual days in month
        } else {
            // Full Months
            if (method === DepreciationMethod.STRAIGHT_LINE) {
